...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.SpatialKeyboard
{
    ...
    /// </summary>
    public class XRKeyboardDisplayMB : MonoBehaviour
    {
        ...
        public bool constrainYawOnly = true;
        ...
        void Update()
        {
            if ( m_ActiveKeyboard == null || !m_ActiveKeyboard.isOpen) return;
        ...
        void ComputeTargetPose(out Vector3 targetPos, out Quaternion rotation)
        {
            if (positionSource == null) { targetPos = transform.position; rotation = m_ActiveKeyboard != null ? m_ActiveKeyboard.transform.rotation : Quaternion.identity; return; }
            Vector3 forward = positionSource.forward;
            if (constrainYawOnly) { forward.y = 0f; if (forward.sqrMagnitude < 1e-4f) forward = positionSource.forward; }
            forward.Normalize();
            targetPos = positionSource.position + forward * Mathf.Max(0.01f, distance) + Vector3.up * verticalOffset;
            if (faceUser)
            {
                Vector3 toUser = (positionSource.position - targetPos);
                if (constrainYawOnly) toUser.y = 0f;
                if (toUser.sqrMagnitude < 1e-4f) toUser = -forward;
                rotation = Quaternion.LookRotation(toUser.normalized, Vector3.up);
            }
            else rotation = m_ActiveKeyboard != null ? m_ActiveKeyboard.transform.rotation : Quaternion.identity;
        }
        ...
        void OnEnable()
        {
            if (m_InputField != null)
                m_InputField.onSelect.AddListener(OnInputFieldGainedFocus);
          /*  ComputeTargetPose(out var targetPos, out var rot);
            m_ActiveKeyboard.transform.position = Vector3.SmoothDamp(m_ActiveKeyboard.transform.position, targetPos, ref _vel, Mathf.Max(0.0001f, smoothTime));
            m_ActiveKeyboard.transform.rotation = Quaternion.Slerp(m_ActiveKeyboard.transform.rotation, rot, Time.deltaTime / Mathf.Max(0.0001f, smoothTime));
          */
        }

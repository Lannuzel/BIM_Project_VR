...
public class RaycastSelectAndMoveNW : Fusion.NetworkBehaviour
{
    ...
    private ReservationInteractionHandler reservationInteractionHandler;
    ...
    private void ApplyControllerRotation()
    {
        ...
        Quaternion deltaJoystickRotation = Quaternion.Euler(0f, 0f, 0f);

        if (selectedObject.transform.name.Contains("CylinderMur"))
            deltaJoystickRotation = Quaternion.Euler(0f, 0f, yaw);   
        else
            deltaJoystickRotation = Quaternion.Euler(0f,  yaw, 0f);
        ...
        Quaternion deltaRollRotation = Quaternion.Euler(0, 0, deltaRoll);
        if (selectedObject.transform.name.Contains("CylinderMur"))
            deltaRollRotation = Quaternion.Euler(0, 0, deltaRoll);
        else
            deltaRollRotation = Quaternion.Euler(0, deltaRoll, 0f);
        ...
        Debug.LogError("trying to rotate object");
    }
